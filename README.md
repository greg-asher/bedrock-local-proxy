...
The `report` command uses the current selected or default configuration to estimate every eligible request. It never falls back to a stored request-time cost: missing usage, missing prices, unresolved upstream model IDs, and conflicting duplicate prices remain unavailable and appear under pricing blockers. The report requires a usable configuration. Costs are estimates, not final AWS billing.
...
Generate a standalone HTML dashboard from locally stored session events. It includes static spend and request trends, model-spend and token-mix donuts, failure causes, plus a month-end run-rate based on calendar days elapsed in the month containing the latest request. The run-rate is a projection of known estimated cost, never final AWS billing. Reports use current prices and require a usable configuration.
